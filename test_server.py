...
class TestGetContacts:
    ...
    def test_get_all_contacts(self, client, sample_contacts):
        ...
        data = response.get_json()
        assert len(data) == 4
        # Проверяем, что избранные идут первыми
...
class TestToggleFavorite:
    ...
    def test_toggle_favorite_to_false(self, client, sample_contacts):
        ...
        # Находим контакт, который в избранном
        response = client.get('/api/contacts')
        contacts = response.get_json()
...
class TestUpdateOrder:
    ...
    def test_update_order_success(self, client, sample_contacts):
        ...
        # Убеждаемся, что есть хотя бы 2 контакта для изменения порядка
        assert len(original_order) >= 2, "Нужно минимум 2 контакта для теста изменения порядка"
        
        # Важно: контакты сортируются сначала по is_favorite DESC, потом по order_index ASC
        # Поэтому избранные всегда идут первыми, затем обычные
        ...
        # Меняем порядок внутри каждой группы на обратный
        ...
        # Новый порядок: сначала избранные (в обратном порядке), потом обычные (в обратном порядке)
        new_order = new_favorites + new_regular
        
        # Убеждаемся, что порядок действительно изменился (если есть хотя бы 2 контакта в одной группе)
        ...
        # Проверяем порядок внутри группы избранных
        ...
        # Проверяем порядок внутри группы обычных
